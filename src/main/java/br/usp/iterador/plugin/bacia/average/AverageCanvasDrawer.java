...
class AverageCanvasDrawer {
	...
	public AverageCanvasDrawer(AveragesInfo info, int w, int h,
			GUIHelper helper, BasinController controller) {
		this.w = w;
		this.h = h;
		this.helper = helper;
		this.info = info;
	...
	private void drawLine(Graphics2D g, double[] p1, double[] p2) {
		...
		g.drawLine((int) helper.mudaEscala(p1[0], xScale, 0, w), h
				- (int) helper.mudaEscala(p1[1], yScale, 0, h), (int) helper
				.mudaEscala(p2[0], xScale, 0, w), h
				- (int) helper.mudaEscala(p2[1], yScale, 0, h));
	}
	...
	public void drawCurrentPolygon(Graphics2D g) {
		...
		if (p != null && p.getPoints().size() != 0)
			drawPolygon(g, p.getPoints(), Color.YELLOW);
	}
