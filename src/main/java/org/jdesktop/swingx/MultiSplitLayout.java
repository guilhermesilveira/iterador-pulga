...
public class MultiSplitLayout implements LayoutManager {
	 ...
	 * <code>child</code>. After layoutContainer() recomputes the bounds of
	 * all of the nodes in the model, it will set this child's bounds to the
	 * bounds of the Leaf node with <code>name</code>. Note: if a component
	 * was already added with the same name, this method does not remove it from
	 * its parent.
	 * 
	 * @param name
