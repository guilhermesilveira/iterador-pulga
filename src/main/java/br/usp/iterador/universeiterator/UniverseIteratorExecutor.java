...
public class UniverseIteratorExecutor {
	...
	private static final Logger logger = Logger
			.getLogger(UniverseIteratorExecutor.class);
	...
	public void doSquare() {
		...
		while (true) {
			...
			if (x < width && y < height && !done[y][x]) {
				...
				double rightX = helper.mudaEscala(x + 1, 0, width, app
						.getXScale());
				double rightY = helper.mudaEscala(y + 1, 0, height, app
						.getYScale());
				listener.doPoint(leftX, leftY, rightX, rightY, curY, curX,
						delY, delX, x, y);
				pointsDone++;
				done[y][x] = true;
			}
			...
			if (x < width) {
				...
				curX = curY = 0;
				logger.debug(String
						.format("del changed to (%d,%d)", delX, delY));
			}
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (!done[y][x]) {
					...
					double leftX = helper.mudaEscala(x, 0, width, app
							.getXScale());
					double leftY = helper.mudaEscala(y, 0, height, app
							.getYScale());
					double rightX = helper.mudaEscala(x + 1, 0, width, app
							.getXScale());
					double rightY = helper.mudaEscala(y + 1, 0, height, app
							.getYScale());
					listener.doPoint(leftX, leftY, rightX, rightY, y, x, 0, 0,
							x, y);
					pointsDone++;
					done[y][x] = true;
				}
